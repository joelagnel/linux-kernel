		...
		if (cmd_desc[i].type == CAM_CMD_BUF_FW) {
			...
			*fw_cmd_buf_iova_addr = addr;
			...
			if (cmd_desc[i].offset >= len ||
				...
				cmd_desc[i].size)){
				CAM_ERR(CAM_ICP, "Invalid offset/length");
				CAM_ERR(CAM_ICP,
					"i %d offset 0x%x len 0x%lx size 0x%x",
					i, cmd_desc[i].offset,
					len, cmd_desc[i].size);
				goto rel_cmd_buf;
			}
	...
	int32_t *idx)
{
	...
	if (ctx_data->hfi_frame_process.frame_info[index].io_config)  {
		hfi_cmd = &ctx_data->hfi_frame_process.frame_info[index]
			.hfi_cfg_io_cmd;
		rc = cam_icp_mgr_process_cfg_io_cmd(ctx_data, hfi_cmd,
			packet->header.request_id,
			ctx_data->hfi_frame_process.frame_info[index]
			.io_config);
	}
	*idx = index;
