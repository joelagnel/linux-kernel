...
MODULE_PARM_DESC(perf_type, "Type of test (rcu, srcu, refcnt, plain, prwsew, rwsem, rwlock)");
...
enum perf_type { RCU, SRCU, REFCNT, PLAIN, PRWSEM, RWSEM, RWLOCK };
...
/* Number of readers that are part of the experiment. */
...
repeat:
	...
	// Make sure that the CPU is affinitzed appropriately during testing.
...
static int main_func(void *arg)
{
	...
	PERFOUT("END OF EXPERIMENTS. Printing average duration per loop (nanoseconds)...\n");
	PERFOUT("TOTAL LOOPS PER READER: %ld\n", loops);

	buf[0] = 0;
	strcat(buf, "Threads Time\n");
